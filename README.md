...
- Добавлены защиты (senior review 2026): жёсткий лимит длины входящего текста (1800 символов), простой rate-limit ~1 ответ в 3.2с на чат, запись learning вынесена из event loop'а. Бот может молча пропускать очень частые сообщения — это нормально и защищает от спама/счетов.
